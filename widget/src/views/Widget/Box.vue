...
export default defineComponent({
  ...
  setup (_, { emit }: SetupContext): SetupReturn {
    ...
    const label = computed<string>(() => {
      ...
      const retorno = labels[store.feedbackType] || labels.DEFAULT
      return retorno
    })
...
.box {
  @apply fixed z-50 bottom-0 right-0 mb-5 mr-5 rounded-xl
    p-5 px-5 flex flex-col items-center shadow-xl select-none
    bg-white;
  width: 400px;
}
</style>
